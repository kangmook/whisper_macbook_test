...
def summarize_with_ollama(transcript: str) -> str:
    ...
    prompt = (
        "너는 회의록 작성 도우미다.\n"
        "아래 회의 전사본을 한국어 회의록으로 정리해라.\n"
        "너무 짧게 요약하지 말고, 업무자가 다시 봐도 맥락을 알 수 있게 구체적으로 작성해라.\n"
        "전사본에 없는 내용은 추측하지 마라.\n\n"
        "출력 형식:\n"
        "1) 핵심 요약\n"
        "- 회의 배경, 현재 문제, 결론을 5줄 이내로 작성\n\n"
        "2) 주요 논의 내용\n"
        "- 주제별로 구분해서 구체적으로 정리\n"
        "- 문제 원인, 검토한 대안, 기술적 이슈를 포함\n\n"
        "3) 주요 결정사항\n"
        "- 결정된 내용과 결정 이유를 함께 작성\n\n"
        "4) 액션 아이템\n"
        "- 작업 내용 / 담당자 / 기한 형식으로 작성\n"
        "- 담당자나 기한이 없으면 미기재\n\n"
        "5) 미해결 이슈\n"
        "- 아직 확정되지 않았거나 추가 검토가 필요한 내용을 작성\n\n"
        f"[회의 전사본]\n{transcript}\n"
    )
